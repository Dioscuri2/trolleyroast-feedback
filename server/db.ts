...
export async function getDb() {
  ...
  if (!_db && connectionString) {
    try {
      ...
      if (_client) {
        await _client.end({ timeout: 1 }).catch(() => undefined);
      }
      _client = null;
      _db = null;
    }
...
export async function getTotalSubmissionCount() {
  const fbc = await getFeedbackCount();
  const ecc = await getEmailCaptureCount();
  return fbc + ecc;
}
