      ...
      <main className="max-w-4xl mx-auto px-4 py-14 sm:py-20">
        <div className="max-w-2xl mb-10">
          ...
          <p className="text-base leading-relaxed text-[#6B6860]">
            This route is scaffolded for future supermarket-specific landing pages. For now, it explains the current public comparison footprint and gives search engines a stable destination.
          </p>
        </div>
          ...
          {SUPERMARKETS.map((name) => (
            <div key={name} className="rounded-2xl border border-[#E8E3D9] bg-white p-5 flex items-center gap-3 shadow-sm">
              <div className="w-10 h-10 rounded-xl bg-[#1B3A2D]/8 flex items-center justify-center">
                <Store size={18} className="text-[#1B3A2D]" />
              </div>
              <div>
                <p className="text-sm font-semibold text-[#1C1A17]">{name}</p>
                <p className="text-xs text-[#7A7570]">Whole-basket comparison supported in public marketing messaging.</p>
              </div>
            </div>
          ))}
        </div>
